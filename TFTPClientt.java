...
public class TFTPClientt {
	...
	public void sendData() {
		...
		try {
			...
			for (int datablck = 1; input.read() != -1; datablck++) {
	...
	public void sendReceive() {
		...
		for (int i = 0; i <= 10; i++) {
			...
			if (i == 10)
				message[1] = 8; // if it's the 8th time, send an invalid request
			...
			try {
				sendPacket = new DatagramPacket(message, length, InetAddress.getLocalHost(), sendPort);
