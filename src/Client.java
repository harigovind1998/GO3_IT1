...
public class Client {
	...
	public static void main(String[] args) {
		...
		for (;;) {
			...
			DatagramPacket receive = new DatagramPacket(client.in, client.in.length);
			try {

				client.sendReceiveSocket.receive(receive);
				...
				if (client.in[1] == 3) {
					...
					if (receive.getLength() >= 512) { // check if blocks at least 512
					...
					// send data if acknowledgment block is correct
					if (com.CheckAck(com.recievePacket(client.sendReceiveSocket,100 ), blocknum) == true) {
						client.sendData();
